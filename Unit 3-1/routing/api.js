...
router.get("/get-right-arm-spin-players", (req, res) => {
  baseController.find(
...
router.get("/get-players-by-least-wickets", (req, res) => {
  baseController
    .find({})
    .sort({ Wickets: 1 })
    .exec((err, allPlayersDetails) => {
...
router.get("/get-players-by-left-handed", (req, res) => {
  baseController
    .find({ Batting: "Left Handed" })
    .exec((err, allPlayersDetails) => {
      if (err) {
        return res.send({ response: err });
      }
